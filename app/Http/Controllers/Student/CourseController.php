...
class CourseController extends Controller
{
    ...
    public function learn(Course $course, Lesson $lesson = null)
    {
        ...
        if ($quizId && !$lesson) {
            $quiz = \App\Models\Quiz::with('questions.options')->find($quizId);
            if ($quiz) {
        ...
        if (!$lesson) {
            if (!$enrollment) {
                ...
                $lesson = \App\Models\Lesson::whereHas('section', function($q) use ($course) {
        ...
        if (!$enrollment) {
            ...
            if ($lesson instanceof \App\Models\Lesson && !$lesson->is_preview) {
            ...
            if ($lesson instanceof \App\Models\Quiz && !$lesson->is_preview) {
                abort(403, 'You must be enrolled to view this content.');
            }
            // If it's neither a lesson nor a quiz, or if it's a quiz without is_preview, deny access
            if (!($lesson instanceof \App\Models\Lesson) && !($lesson instanceof \App\Models\Quiz)) {
                abort(403, 'You must be enrolled to view this content.');
            }
        }
    ...
    public function completeLesson(Lesson $lesson)
    {
        ...
        app(\App\Services\CourseCompletionService::class)->checkAndComplete($enrollment);
     ...
     */
    public function playQuiz(\App\Models\Quiz $quiz)
    {
